app.controller("AddFee-ct", ['$scope', '$state','$scope', '$window', 'serF', function ($scope, $state,$window, serF){
    ...
    $scope.insertPWFFunc = function (srch) {
        ...
        if ((srch.date1 == '' || srch.date1 == undefined) && srch.date2 != undefined) {

            for (var i = 1; i <= 12; i++) {
                monthArr.push(i);
                console.log(monthArr)
            }
            param["Month"] = monthArr;
        }
        else {
            monthArr.push(srch.date1);
            param["Month"] = monthArr;
        }

        var serviceData = serF.serverCall(param, $scope.insertPWF);
        serviceData.then(function (result) {
