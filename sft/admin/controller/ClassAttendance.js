app.controller("ClassAttendance-ct", ['$scope', '$state', '$scope', '$window', 'serF', function ($scope, $state, $window, serF) {
    ...
    $scope.checkall1 = function () {        
        ...
        angular.forEach($scope.allFeeArray, function (cate1) {            
            ...
            if ($scope.seletedones[cate1.std_id] == true) {
                ...
                var qObj = { "stdId": cate1.std_id, "isPresent": 1, "clsId": cate1.cls_id }
                if (qObj != '')
    ...
    $scope.checkall2 = function () {
       $scope.seleteditems = [];
        ...
        angular.forEach($scope.allFeeArray, function (cate2) {
            ...
            $scope.seletedones2[cate2.std_id] = $scope.object2.selected2;
            if ($scope.object2.selected2 == false || $scope.object2.selected2 == undefined) {
                $scope.count = 0;
                $scope.seleteditems = [];
            }
            if ($scope.seletedones2[cate2.std_id] == true) {
                $scope.count++;
                var qObj = { "stdId": cate2.std_id, "isPresent": -1, "clsId": cate2.cls_id }
    ...
    $scope.checkall3 = function () {
        ...
        angular.forEach($scope.allFeeArray, function (cate3) {
            ...
            $scope.seletedones3[cate3.std_id] = $scope.object2.selected3;
            if ($scope.object2.selected3 == false || $scope.object2.selected3 == undefined) {
                $scope.count = 0;
                $scope.seleteditems = [];
            }
            if ($scope.seletedones3[cate3.std_id] == true) {
                $scope.count++;
                var qObj = { "stdId": cate3.std_id, "isPresent": 2, "clsId": cate3.cls_id}
   ...
   $scope.selectchecked3 = function () {
        ...
        angular.forEach($scope.allFeeArray, function (cate3) {
            if ($scope.seletedones3[cate3.std_id] == true) {
                 qObj = { "stdId": cate3.std_id, "isPresent": 2, "clsId": cate3.cls_id }
   ...
   $scope.resetClassAttendanceFunc = function () {
       ...
       if ($scope.srch != undefined && $scope.srch != "" && $scope.srch.classId != undefined && $scope.srch.classId != "") { param["classId"] = $scope.srch.classId.cls_id; }


       var serviceData = serF.serverCall(param, $scope.resetClassAttendance);
    ...
    $scope.addAttendanceFunc = function () {
        ...
        var param = {
         ...
         var serviceData = serF.serverCall(param, $scope.insertAttendance);
        serviceData.then(function (result) {
            $scope.spinner = false;
            $scope.updateStdFeeArr = result.data.data;
            $state.reload();
    ...
    $scope.getAllAttendanceFunc = function (srch) {
        ...
        var serviceData = serF.serverCall(param, $scope.getAttendance);
        serviceData.then(function (result) {
