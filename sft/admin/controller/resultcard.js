app.controller("resultcard", ['$scope', '$timeout', '$state', '$scope', '$window', 'serF', function($scope, $timeout, $state, $window, serF) {
    ...
    $scope.circularText = function(txt, radius, classIndex) {
        ...
        txt.forEach((ea) => {
            ea = `<p style='height:300px;position:absolute;transform:rotate(${origin}deg);transform-origin:0 100%'>${ea}</p>`;
    ...
    $scope.getAllStudentsFunc = function(aObj) {
        ...
        if (aObj) { param["classId"] = aObj.classId.cl_id; }
